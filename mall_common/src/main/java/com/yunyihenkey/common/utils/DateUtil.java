...
public class DateUtil {
	...
	/**
	 * 
	 * @desc 对日期为空数据进行0填充数据，比如用于：时段分析、日段分析
	 * @auth josnow
	 * @date 2017年3月16日 上午11:43:22
	 * @param list
	 *            查询结果
	 * @param startDate
	 *            开始时间
	 ...
	 *             异常
	 */
	public static void fillZero(List<Map<String, Object>> list, Date startDate, Date endDate, int dateTypeField,
			String dateFormat, String key, String[] keysValue) throws ParseException {
		...
		if (startDate != null) {
			...
			startCalendar.setTime(sdf.parse((String) list.get(0).get(key)));
		}
		...
		while (listIterator.hasNext()) {
			...
			// list中下一个对象
			Map<String, Object> nextMap = list.get(listIterator.nextIndex());

			// 时间字符串
			String nextDayteItem = (String) nextMap.get(key);

			// 下一个日期不为空才对下一个日期之前的时间进行填充
			if (StringUtils.isNotEmpty(nextDayteItem)) {
				String newItem;
				while (!nextDayteItem.equals(newItem = sdf.format(startCalendar.getTime()))) {
					// 进行0填充
					Map<String, Object> map = new LinkedHashMap<>();
					map.put(key, newItem);
					for (String k : keysValue) {
						map.put(k, 0);
					}
		...
		while ((calendarDate = startCalendar.getTime()).getTime() < endDate.getTime()) {
			// 进行0填充
			Map<String, Object> map = new LinkedHashMap<>();
			map.put(key, sdf.format(calendarDate));
			for (String k : keysValue) {
				map.put(k, 0);
			}
			listIterator.add(map);
